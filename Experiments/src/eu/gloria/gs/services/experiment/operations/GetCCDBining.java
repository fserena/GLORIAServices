 ...
 * 
 */
public class GetCCDBining extends ServiceOperation {
	...
	public void execute() throws ExperimentOperationException {
		try {
			...
			try {
				binX = this.getCCDTeleoperation().getBiningX(rtName, camName);
			...
			try {
				binY = this.getCCDTeleoperation().getBiningX(rtName, camName);
			} catch (CCDTeleoperationException e) {
				throw new ExperimentOperationException(e.getMessage());
			} catch (DeviceOperationFailedException e) {
			...
			this.getContext().getExperimentContext()
					.setParameterValue(binXParameter, binX);
			this.getContext().getExperimentContext()
					.setParameterValue(binYParameter, binY);
